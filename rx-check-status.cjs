...
(async () => {
  ...
  page.on('response', async (r) => {
    if (r.url().includes('/prescription-status')) {
      console.log('STATUS ->', await r.text().catch(() => '(err)'));
    }
  });
  ...
  await closeAnyModal(page);
  await page.screenshot({ path: '/tmp/claude-1000/-home-noname-Documentos-desenvolvimento-dev/9e017e0c-f1c7-4380-b92d-8d052b150958/scratchpad/rx-check-cart.png', fullPage: true });
