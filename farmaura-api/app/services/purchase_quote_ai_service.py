...
class PurchaseQuoteAiService:
    ...
    def _quote_extraction_prompt(self, file_name: str) -> str:
        ...
        return (
            ...
            "nao sao itens cotados. Algumas planilhas repetem varios blocos de itens, cada um "
            "iniciado por uma linha curta sem preco ao lado (ex.: 'AMARELOU GERAL ?', 'DESMAIA "
            "SEM DO NEM PIEDADE', 'CRESPO'), seguida de um cabecalho de colunas "
            "(Descricao/Cod./Qtd/Valor ou similar) e so entao as linhas de itens do bloco: essa "
            "linha de titulo e o nome da linha/colecao de produtos daquele bloco, nao a marca do "
            "fornecedor — combine-a com a descricao de cada item do bloco em vez de trata-la como "
            "brand_name (ex.: titulo 'AMARELOU GERAL ?' + item 'Sh. 300ml' vira description "
            "'Amarelou Geral? - Sh. 300ml'), e continue extraindo normalmente cada item de cada "
            "bloco, sem pular nenhum. brand_name e o fabricante/marca do catalogo inteiro — "
            "normalmente o mesmo do inicio ao fim do arquivo. NUNCA use como brand_name uma "
            "dessas linhas de titulo de bloco/colecao, nem um titulo generico do documento como "
            "'CATALOGO DE PRODUTOS', 'TABELA DE PRECOS' ou 'LISTA DE PRECOS' — esses so descrevem "
            "o documento em si, nao uma marca. Se o conteudo da planilha nao tiver um nome de "
            "marca/fabricante explicito, procure no nome do arquivo: quando ele tiver uma palavra "
            "ou expressao que pareca claramente um nome de marca (ex.: arquivo 'TABELA OHMY 2026' "
            "sugere a marca 'Oh My'; ignore palavras genericas do nome do arquivo como 'tabela', "
            "'lista', 'catalogo', 'precos', 'reajuste', datas e numeros de versao), use essa marca "
            "como brand_name em todos os itens do catalogo; so deixe brand_name vazio quando nem "
            "o conteudo nem o nome do arquivo derem nenhuma pista razoavel de marca. "
            "Se o mesmo produto (mesmo codigo ou EAN) aparecer em mais de "
