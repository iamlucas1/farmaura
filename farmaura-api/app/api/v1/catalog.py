...
async def list_catalog(
    ...
    """Return the tenant-scoped authenticated catalog listing."""

    service = CatalogService(session=session)
    return await service.list_products(tenant_id=str(subject.tenant_id), page=page, page_size=page_size)


@router.get("/public", response_model=PublicCatalogListResponse)
