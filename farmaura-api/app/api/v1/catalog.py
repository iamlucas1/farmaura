...
@router.get(
    "/products/{product_id}/also-bought",
    response_model=AlsoBoughtProductsResponse,
    dependencies=[Depends(rate_limit(PUBLIC_RATE_LIMIT))],
)
async def list_also_bought_products(
    product_id: str,
    limit: int = Query(default=10, ge=1, le=20),
    session: AsyncSession = Depends(get_session),
) -> AlsoBoughtProductsResponse:
    ...
    service = CatalogService(session=session)
    return await service.list_also_bought_products(product_id=product_id, limit=limit)
