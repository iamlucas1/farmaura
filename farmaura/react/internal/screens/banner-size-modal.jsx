...
function SizePreview({ width, height }) {
  ...
  return (
    <div style={{ width: PREVIEW_MAX_W, height: PREVIEW_MAX_H, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: boxW, height: boxH, borderRadius: "var(--radius-sm)", overflow: "hidden" }}>
...
function BannerSizeModal({ open, value, onCancel, onApply }) {
  ...
  return (
    <Modal
      open={open} onClose={onCancel} title="Escolher tamanho do banner" wide
      subtitle="Mesma arte de exemplo, cortada em cada proporção — dá pra comparar visualmente antes de decidir. Todo slide de imagem novo (ou ajustado) usa o tamanho escolhido aqui."
      footer={(
        <>
          <button className="btn btn-secondary" onClick={onCancel}>Cancelar</button>
          <button className="btn btn-primary" style={{ flex: 1, justifyContent: "center" }} onClick={() => onApply(finalWidth, finalHeight)}>
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14 }}>
        {PRESETS.map((preset) => (
          <button
            ...
            style={{
              ...
              border: "1.5px solid " + (selectedId === preset.id ? "var(--accent)" : "var(--border)"),
              background: selectedId === preset.id ? "var(--accent-soft)" : "var(--surface)",
              borderRadius: "var(--radius-lg)", padding: 12, cursor: "pointer", textAlign: "center",
            }}
          >
            <SizePreview width={preset.width} height={preset.height} />
            <div style={{ fontWeight: 700, fontSize: 14 }}>{preset.label}</div>
            <div className="cell-muted" style={{ fontSize: 12 }}>{preset.width} × {preset.height}px</div>
            <div className="cell-muted" style={{ fontSize: 11.5 }}>{preset.desc}</div>
          </button>
        ))}
        ...
        <button
          ...
          style={{
            display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
            border: "1.5px dashed " + (selectedId === "custom" ? "var(--accent)" : "var(--border)"),
            background: selectedId === "custom" ? "var(--accent-soft)" : "var(--surface)",
            borderRadius: "var(--radius-lg)", padding: 12, cursor: "pointer", textAlign: "center",
          }}
        >
          <SizePreview width={finalWidth} height={finalHeight} />
          ...
          {selectedId === "custom" && (
            <div style={{ display: "flex", gap: 6, marginTop: 2 }} onClick={(e) => e.stopPropagation()}>
              <input
                className="input" type="number" min="320" max="3840" step="10" value={customWidth}
                onChange={(e) => setCustomWidth(e.target.value)}
                style={{ width: 76, fontSize: 12.5, padding: "6px 8px" }}
              />
              <span style={{ alignSelf: "center", color: "var(--text-muted)" }}>×</span>
              <input
                className="input" type="number" min="120" max="1600" step="10" value={customHeight}
                onChange={(e) => setCustomHeight(e.target.value)}
                style={{ width: 76, fontSize: 12.5, padding: "6px 8px" }}
              />
            </div>
          )}
          {selectedId !== "custom" && <div className="cell-muted" style={{ fontSize: 11.5 }}>Defina sua própria largura e altura.</div>}
        </button>
      </div>
    </Modal>
  );
