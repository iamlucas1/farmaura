...
- 2026-09-20: novas tabelas fiscais (`fiscal_events`, `fiscal_attempts`, `fiscal_number_sequences`, `fiscal_inutilizations`, `product_fiscal_profiles`) entram na lista de tabelas com RLS por tenant. As quatro primeiras têm `tenant OU is_system_job()` (o worker de emissão é código de servidor confiável, como o `fiscal_documents`); `product_fiscal_profiles` fica só por tenant. Aplicado por `row_level_security.py` no bootstrap; **ainda não executado em Postgres real** ([[../06_Pendencias/aplicar-migration-nfce-fiscal-em-producao|pendência]]).
