...
- 2026-09-20: modal "completar cadastro" (`.fa-nudge*` em `marketplace.css`) redesenhada: faixa de topo em `--fa-rose-soft` com os arcos aura, título curto, pílulas 2×2 feito/pendente e um único botão principal; só tokens existentes. O ícone `gift` do conjunto desenha uma grade que lê como tabela — para "ofertas" usar `percent` ou `tag`. Ver [[../00_Decisoes/2026-09-20-modal-completar-cadastro-enxuta-com-progresso|ADR]].
