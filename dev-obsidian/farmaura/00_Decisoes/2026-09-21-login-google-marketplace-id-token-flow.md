...
- **Não verificado**: um login real via Google (precisa de consentimento humano num navegador de verdade) — bloqueado localmente por origem não autorizada no Google Cloud Console (`http://127.0.0.1:3000` ainda não estava na lista de origens JavaScript autorizadas no momento do teste). Usuário deve confirmar essa origem (e as de staging/produção) no Console antes do primeiro teste ponta a ponta real.
