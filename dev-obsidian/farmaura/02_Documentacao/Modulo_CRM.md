...
- 2026-09-21: nudge de completar cadastro (`ProfileCompletionNudge`/`GoogleWelcomeProfileModal`) passou a cobrir também **telefone, CPF e data de nascimento** — antes só cobria gênero/estado civil/filhos/endereço, então uma conta criada via Google (que só traz nome/e-mail) nunca era cobrada por esses três. Novo checkbox **"Não tenho filhos"** em Meu perfil marca `children_count=0` explicitamente (já contava como resposta completa no backend — `app/domain/profile_nudge.py` — só faltava um jeito de escolher isso na UI em vez de digitar "0"). **CPF continua obrigatório para salvar qualquer edição de perfil** (`PUT /customers/me/profile`, `CustomerProfileUpdateRequest.cpf` com `min_length=11`) — decisão explícita do usuário: o cliente precisa se identificar por CPF no balcão da loja física, então uma conta sem CPF nunca fica "parcialmente completa" salvando só os outros campos. Ver [[../06_Pendencias/cpfs-do-seed-invalidos-impedem-salvar-perfil|pendência relacionada]] (CPFs do seed não passam nessa validação).
- 2026-09-21: conta criada via login com Google (que só traz nome/e-mail, nenhum outro dado) passou a ver, na primeira sessão, uma modal própria "Complete seu cadastro" (`GoogleWelcomeProfileModal`) em vez da "Ofertas feitas pra você" (`ProfileCompletionNudge`) — mesma lista de campos faltantes e mesmo dismiss/snooze de servidor (`profile_nudge_dismissed_at`), sem o efeito colateral de opt-in em promoções que a modal de ofertas tem. Sinalizada por `AuthenticatedResponse.is_new_google_account` (só verdadeiro na resposta exata que acabou de criar a conta), guardada só em memória no frontend (`justSignedUpViaGoogle`, `marketplace-app.jsx`) — nunca persistida, então não reaparece em sessões seguintes. Ver [[../00_Decisoes/2026-09-21-login-google-marketplace-id-token-flow|ADR]].
